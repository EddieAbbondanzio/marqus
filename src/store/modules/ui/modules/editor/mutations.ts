...
export class EditorMutations extends Mutations<EditorState> {
  ...
  SET_NOTEBOOK_DROPDOWN_VISIBLE({
    value: { tab, visible }
  }: UndoPayload<{ tab: Tab; visible: boolean }>) {
    tab.notebookDropdownVisible = visible;
  }

  SET_TAG_DROPDOWN_VISIBLE({
    value: { tab, visible }
  }: UndoPayload<{ tab: Tab; visible: boolean }>) {
  ...
  MOVE_TAB(p: UndoPayload<number>) {
    ...
    const oldIndex = this.state.tabs.values.findIndex(
      t => t.id === this.state.tabs.dragging!.id
    );
  ...
  OPEN_TAB(
    p: UndoPayload<{ noteId: string; content: string; preview: boolean }>
  ) {
    // See if we already opened the tab
    const existing = this.state.tabs.values.find(
      t => t.noteId === p.value.noteId
    );
    ...
    if (existing != null) {
      ...
      if (
        ...
        this.state.tabs.active === existing.id
      ) {
    ...
    const existingPreviewTabIndex = this.state.tabs.values.findIndex(
      t => t.state === "preview"
    );
